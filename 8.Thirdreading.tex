\chapter{Beyond \emph{de re \dash de dicto}\,: The Third Reading}\label{cha:beyond_emph_de_re_de_dicto_the_third_reading} 

\chapterprecishere{In this chapter, we will see that quantificational noun phrases in the scope of a modal operator can receive a reading where their restrictive predicate is not interpreted in the worlds introduced by the modal operator (which is what happens in \emph{de re} readings as well) while \emph{at the same time} their quantificational force takes scope below the modal operator (which is what happens in \emph{de dicto} readings as well). This seemingly paradoxical situation might force whole-sale revisions to our architecture. We discuss the standard solution (which involves supplying predicates with world-arguments) and some alternatives. % We end with a historical overview: the phenomenon we discuss here has been noted again and again in the history of semantics but there has been surprisingly little cross-fertilization.
}

\minitoc

\section{A Problem: Additional Readings and Scope Paradoxes}
...
Fodor observes that \Last has three readings, which she labels ``specific \emph{de re},'' ``non-specific \emph{de re},'' and ``non-specific \emph{de dicto}.''
...
We have annotated each predicate with the world in which we wish to evaluate it. $w_0$ is the evaluation world for the entire sentence and it is the world in which we evaluate the predicates \expression{want} and \expression{hat-just-like-mine}. The embedded sentence contributes a function from worlds to truth-values and we insert an explicit $\lambda$-operator binding the world where the predicate \expression{buy} is evaluated. The crucial aspect of \Last is that the world in which \expression{hat-just-like-mine} is evaluated is the matrix evaluation world and not the same world in which its clause-mate predicate \expression{buy} is evaluated. This LF thus looks like it might faithfully capture Fodor's third reading.
...
The sentence would then obviously have an assignment-dependent extension (a truth-value), depending on what world the variable assignment assigns to the world variable with index 12. In our intensional system of Chapter 1\dash5, we were assuming the following principle:
...
Now, we have to look at more complex sentences. First, a simple case of embedding. The sentence is \expression{John wants to leave}, which now as an LF like this:
...
Next, look at an example involving a complex subject, such as \expression{the teacher left}.

\ex. $[\ \lambda w_{0} [ [ \mbox{ the } [ \mbox{ teacher } w_{0} ]] [ \mbox{ left } w_{0} ]]]$

The verb will need a world argument as before. The noun \expression{teacher} will likewise need one, so that \expression{the} can get the required argument of type $\angles{e,t}$ (not $\angles{s,et}$!). If we co-index the two world variables, we derive as the semantic value for \Last what its intension would have been in old system. But nothing we have said forces us to co-index the two world variables, which is what will allow us to derive the third reading for relevant examples.
...
\ex. \a. non-specific \emph{de dicto}:\\
...
Let us look back at the account of \emph{de dicto} readings of raised subjects that we sketched earlier
...
% 
% \ex. I have to return \emph{one of these books}.
% 
...
% \emph{de re} reading \dash is available when we embed \Last in a text
% like \Next, which has an anaphoric pronoun in the subsequent sentence.
% 
% \ex. \label{book} I have to return one of these books. But I am not
% finished with \emph{it}.
% 
% Similar disambiguating effects are observed in \Next and \NNext. By
% themselves, the first sentences in \Next and \NNext are ambiguous
...
% continuations.
% 
% \ex. \label{cand} Two candidates could get hired. They aren't very
% optimistic, though.
% 
% \ex. \label{plumber} Jane wants to marry a plumber. He owns a house.
% 
...
% the second sentence of \ref{book} is not in the right environment to
...
% appears to be the utterance of the first sentence in \ref{book} which
% is responsible for making salient the intended referent of
% \emph{it}. How does this work?
% 
% Well, if the first sentence in \ref{book} is read \emph{de} \emph{re},
...
% return. This indeed is how the text in \ref{book} appears to be
% understood.
% 
% We have shown, then, that a \emph{de re} disambiguation of the first
% sentence of \ref{book} provides an appropriate context for the
...
% \ref{book} to \Next.
% 
% \ex. \label{rev} One of the reviewers has read every abstract. I have
% talked to her.
% 
% In isolation, the first sentence of \ref{rev} is scopally ambiguous,
...
% available with the continuation in \ref{rev}. The need to identify a
...
% semantic structures of \ref{rev} and \ref{book} as completely parallel
...
% following variants of \ref{book}, \ref{cand}, and \ref{plumber}.
% 
% \ex. \label{book2} I have to return one of these books. I am supposed
% to drop it off tomorrow.
% 
% \ex. \label{cand2} Two candidates could get hired. They could get
% half-time positions.
% 
% \ex. \label{plumber2} Jane wants to marry a plumber. He has to own a
% house.
% 
...
% examples and the ones in \ref{book}-\ref{plumber} is that here we have
...
% dicto} readings in \ref{book2}-\ref{plumber2} is an application (or
...
% \ref{book2}-\ref{plumber2}, we will propose, in a nutshell, that they
...
% truth-conditions of \ref{book2}.
% 
% \ex. For every accessible world $w$: in $w$, I return one of these
% books. And moreover, for every accessible world $w$: in $w$, I drop
% \emph{the book I return in $w$} off tomorrow.
% 
% The underlined definite in \Last corresponds to the pronoun \emph{it}
% in \ref{book2}. It picks out a unique book for each (accessible) world
...
% A, B, and C. In the first sentence of \ref{book2}, read \emph{de
...
% A, in others B, in yet others C. The second sentence of \ref{book2}
...
% actually get for \ref{book2} when we read the first sentence \emph{de
...
% 
% Suppose then that the LF of the second sentence of \ref{book2} looks
% like this:
% 
...
% one book. However, if we take the first sentence of \ref{book2} to be
...
% the first sentence of \ref{book2}.) Since, $r$ requires as its first
...
% 
%     \ex. Mary believes that John finds a unicorn and he eats it
% 
%     [\dots]
% 
...
% 
%     \ex. John seeks a unicorn and Mary seeks it
% 
%     \ex. \label{uni} John tries to find a unicorn and wishes to eat it
% 
%     [\dots]
% 
...
% \ref{uni}. Let him, however, and those who might sympathize with him
% consider the following variant of \ref{uni} and attempt to make
% nonreferential sense of it:
% 
%     \ex. \label{uni2} John wishes to find a unicorn and tries to eat
% it
% 
%   \end{quote}
% 
...
% 
%   Second, there is the claim that \ref{uni} has only a ``referential''
...
% is interesting here is that Montague switches to \ref{uni2} to obtain
% a clearer judgment. He seems to assume that any reasonable theory that
% predicted a ``nonreferential'' reading for \ref{uni} would have to do
% the same for \ref{uni2}. Is this assumption justified? In particular,
% if the ``nonreferential'' reading of \ref{uni} were to be given an
...
% would not automatically arise for \ref{uni2}? \eex
% 
% \end{exercise}
