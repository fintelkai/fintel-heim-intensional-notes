...
\xe
%
\note{We know from the previous chapter exactly in what sense this meaning is
  simplified: it doesn't capture the interplay of beliefs and preferences.}%
...
\ex\null
[a book about soccer] (1 [Emma wanted Taylor to buy $t_1$])
\xe
%
...
\emph{Quantifier scope}: Since \emph{want} is a universal quantifier over worlds
...
The notion of \emph{specificity} is particularly laden with complexity. We use
...
non-specific existential quantification.\note{For discussion, see for example
  \cite{ludlow-neale-1991-indefinites}.} Whether this is merely a pragmatic
...
For example, suppose Mesut thinks that plumbers make ideal spouses, because they
...
breath.\note{Actually, \emph{why} wouldn't one be able to assert both
...
\ex\null [ the abstract-by-you]$_1$ [ Sari believes t$_1$ will-be-accepted] \xe
%	
Calculate the interpretations of the four structures in
\refx{first}\dash(\lastx), and determine their predicted truth-values in each of
the (types of) possible worlds that we described above in our introduction to
the ambiguity.
	
Some assumptions to make the job easier: (i) Assume that \refx{first} and
...
\ex \refx{dere} is true iff $\exists x [x$ is a person in $w_{0}$ \& \\
...
about the evidence when I said \refx{some}. The context we described suggests
...
\refx{some}. What you did understand me to be claiming, apparently, was not
(\lastx) but (\nextx).
...
\textbf{Excursus}. Hopefully the exact analysis of the modal operators
...
%
\note{Some follow up on \cite{fintel-iatridou-2003-ec} can be found in
  \cite{tancredi-2008-multiple, swanson-2010-scope-ecp}}%
...
%
\note{For a thorough investigation of low scope readings of negative DPs,
...
Let us look again at \refx{some} and its apparent surface structure \refx{dere},
both repeated here:
...
\type{st,t}). If it had that type, however, the movement would be semantically
...
\begin{exercise}
  ...
  the trace other than \type{st,t} possible? Why is the movement semantically
...
\subsubsection{3. Higher type for trace of raising, variant 1: type \type{et,t}}
...
turns out to be one other choice besides e that works, namely \type{et,t}:

\ex somebody $\lambda_{2,\type{et,t}}$ [ [ must $f$] [ $t_{2,\type{et,t}}$
have-been-here]]\xe
%
\note{That a trace of type $\type{et,t}$ does not in fact yield the targeted
  non-specific opaque reading had not been noticed until we bothered to
  calculate the meaning of (\lastx). For example, \cite{fox-2000-book}, which
  derives from a dissertation supervised by us, is unaware of the fact that a
  high-type but extensional trace gives a scope-reconstructed but transparent
  reading.}%
...
%	
  ...
  \emph{not} but its trace has the type \type{\type{e,t},t}.
...
\begin{exercise}
  ...
  e and \type{et,t} that would make the structure in \refx{dere}
...
\type{s,\type{\type{e,t},t}}, the type of the intension of a quantifier. As you
...
the trace in subject position is of type \type{s,\type{\type{e,t},t}} and its
sister is of type \type{e,t}. These two cannot combine by either FA or IFA, but
it works if we employ another variant of functional application.%

\ex%
\note{Notice that the problem here is kind of the mirror image of the problem
  that led to the introduction of ``Intensional Functional Application'' in
  H\&K, ch. 12. There, we had a function looking for an argument of type
  \type{s,t}, but the sister node had an extension of type t. IFA allowed us to,
  in effect, construct an argument with an added ``s'' in its type. This time
  around, we have to get rid of an ``s'' rather than adding one; and this is
  what EFA accomplishes.

So we now have three different ``functional application''-type rules altogether
in our system: ordinary FA simply applies $\sv{\beta}^w$ to $\sv{\gamma}^w$; IFA
applies $\sv{\beta}^w$ to $\lambda w'.\sv{\gamma}^{w'}$; and EFA applies
$\sv{\beta}^w(w)$ to $\sv{\gamma}^w$. At most one of them will be applicable to
each given branching node, depending on the type of $\sv{\gamma}^w$.

Think about the situation. Might there be other variant functional application
rules?}%
\emph{Extensionalizing Functional Application} (EFA)\\
If $\alpha$ is a branching node and $\{\beta,\gamma\}$ the set of its daughters,
then, for any world $w$ and assignment $g$: \\
...
\begin{exercise}
  ...
  trace of the subject quantifier is of type \type{s,\type{\type{e,t},t}}.
  \qed
\end{exercise}
...
%
If I say this instead of our original \refx{some} when I come to my office in
...
\xe
%
\note{See \cite{lechner-2007-head} for an early discussion of semantic
...
leave traces of type \type{s,\type{et,t}} and thus get narrow scope
